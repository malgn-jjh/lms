...
if(info.b("target_yn")) {
	...
	else if(1 > webtvTarget.findCount("webtv_id = " + info.i("id") + " AND group_id IN (" + userGroups + ")")) { m.jsError(_message.get("alert.common.permission_view")); return; }
}
if(info.b("category_target_yn")) {
	if(1 > userId) { auth.loginForm(); return; }
	else if(1 > categoryTarget.findCount("category_id = " + info.i("category_id") + " AND group_id IN (" + userGroups + ")")) { m.jsError(_message.get("alert.common.permission_view")); return; }
}
...
try { readArray = m.getCookie("WEBTV_VIEW_" + info.i("category_id")).split("\\,"); } catch(Exception e) { }
