...
//oauth.setDebug(out);
oauth.setClient("google", oklist.s("google_id"), oklist.s("google_secret"));
...
if(!"".equals(code) && !"".equals(state)) {
	...
	if(!oauthMap.containsKey("name") || !oauthMap.containsKey("email")) {
		...
		p.setVar("google_block", true);
		p.setVar("reauth_url", oauth.getAuthUrl("google") + "&auth_type=rerequest");
		p.display();
		return;
	}
	
	//정보
	DataSet info = user.find("login_id = 'google_" + oauthMap.get("id").toString() + "' AND site_id = " + siteId + "");
	if(info.next()) {
		//세션
		mSession.put("login_method", "oauth-google");
		mSession.save();
	...
	mSession.put("join_vendor", "google");
	mSession.put("join_vendor_nm", "구글");
	...
	m.redirect(oauth.getAuthUrl("google"));
}
