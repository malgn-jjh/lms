...
while(list.next()) {
    ...
    if(smsYn) {
        ...
        sms.item("status", 1);
        newSmsId = sms.insert(true);
        if(newSmsId > 0) smsYn = true;
    }

    boolean isEmail = "Y".equals(list.s("email_yn")) && !"".equals(list.s("site_email"));
    int newMailId = 0;
    int mscnt = 0, mfcnt = 0;
    if(list.b("email_yn") && !"".equals(culist.s("site_email"))) {
        String subject = list.s("subject");
        String sender = culist.s("site_email");
        ...
        mail.item("status", 1);
        newMailId = mail.insert(true);
        if(newMailId > 0) isEmail = true;
    }
