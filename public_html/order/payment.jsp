...
pinfo.put("ip", request.getRemoteAddr());
...
loinfo.put("ord_mobile_conv", !"".equals(loinfo.s("ord_mobile")) ? SimpleAES.decrypt(loinfo.s("ord_mobile")) : "");
...
if(
	...
	*/
	...
	if(isDelivery) {
		...
		order.item("ord_phone", !"".equals(m.replace(ordPhone, "-", "")) ? SimpleAES.encrypt(ordPhone) : "");

		String ordMobile = f.glue("-", "ord_mobile1,ord_mobile2,ord_mobile3");
		order.item("ord_mobile", !"".equals(m.replace(ordMobile, "-", "")) ? SimpleAES.encrypt(ordMobile) : "");
		...
		if("Y".equals(f.get("info_modify_yn"))) {
			...
			if(!siteinfo.b("auth_yn") && !siteinfo.b("ipin_yn")) {
				user.item("mobile", !"".equals(m.replace(ordMobile, "-", "")) ? SimpleAES.encrypt(ordMobile) : "");
			}
