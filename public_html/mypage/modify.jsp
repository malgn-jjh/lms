...
if(!isSSO) {
	...
	if(!isAuth) f.addElement("birthday_year", m.time("yyyy", uinfo.s("birthday")), "hname:'생년월일(년도)'");
	if(!isAuth) f.addElement("birthday_month", m.time("MM", uinfo.s("birthday")), "hname:'생년월일(월)'");
	if(!isAuth) f.addElement("birthday_day", m.time("dd", uinfo.s("birthday")), "hname:'생년월일(일)'");
	f.addElement("dept_id", uinfo.i("dept_id"), "hname:'소속'");
	if(!isAuth) f.addElement("gender", uinfo.s("gender"), "hname:'성별'");
	...
	if(!isAuth) f.addElement("mobile1", mobile[0], "hname:'휴대폰번호'");
	if(!isAuth) f.addElement("mobile2", mobile[1], "hname:'휴대폰번호'");
	if(!isAuth) f.addElement("mobile3", mobile[2], "hname:'휴대폰번호'");
	...
	if(m.isPost() && f.validate()) {
		...
		if(!"".equals(passwd)) {
			//제한-비밀번호
			if(!passwd.matches("^(?=.*?[A-Za-z])(?=.*?[0-9])(?=.*?[\\W_]).{8,}$")) {
		...
		if(!isAuth) {
			...
			user.item("mobile", SimpleAES.encrypt(newMobile));
		}
...
if("direct".equals(mSession.s("login_method"))) {
	if(loginId.equals(mSession.s("MODIFY_VERIFY"))) {
		mSession.put("MODIFY_VERIFY", "");
		mSession.save();
	} else {
		m.redirect("modify_verify.jsp");
		return;
	}
}
...
p.setVar("SITE_CONFIG", SiteConfig.getArr(new String[] {"modify_", "user_etc_"}));
