...
while(list.next()) {
	list.put("content_conv", m.cutString(list.s("content"), 150));
	...
	list.put("ktalk_type_conv", m.getItem(list.s("ktalk_type"), ktalk.alltypes));
}

//엑셀
if("excel".equals(m.rs("mode"))) {
	ExcelWriter ex = new ExcelWriter(response, "알림톡관리(" + m.time("yyyy-MM-dd") + ").xls");
	ex.setData(list, new String[] { "__ord=>No", "user_nm=>발송자(성명)","login_id=>회원아이디", "sender=>발송자(휴대전화)", "content=>내용", "send_cnt=>발송건수", "fail_cnt=>실패건수", "reg_date_conv=>등록일"}, "알림톡관리(" + m.time("yyyy-MM-dd") + ")");
	ex.write();
	return;
}

//출력
p.setBody("ktalk.ktalk_list");
p.setVar("query", m.qs());
