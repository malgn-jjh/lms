...
while(list.next()){
	list.put("admin_yn_conv", m.getItem(list.s("admin_yn"), userLogin.adminYnList));
	list.put("login_type_conv", m.getItem(list.s("login_type"), userLogin.loginTypeList));
	list.put("agent_conv", m.cutString(list.s("agent"), 50));
	list.put("reg_date_conv", m.time("yyyy.MM.dd HH:mm:ss", list.s("reg_date")));
}

//엑셀
if("excel".equals(m.rs("mode"))) {
	ExcelWriter ex = new ExcelWriter(response, "접속이력관리(" + m.time("yyyy.MM.dd") + ").xls");
	ex.setData(list, new  String[] { "__ord=>No", "login_id=>로그인아이디", "user_nm=>회원명", "admin_yn_conv=>접속단", "login_type_conv=>구분", "agent=>브라우저", "device=>기기", "ip_addr=>IP", "reg_date_conv=>등록일시" }, "접속이력관리(" + m.time("yyyy.MM.dd") + ")");
	ex.write();
	return;
}
...
p.setVar("date", dinfo);
p.setLoop("admin_yn_list", m.arr2loop(userLogin.adminYnList));
